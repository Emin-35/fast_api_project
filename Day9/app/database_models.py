...
class User(Base):
    ...
    password_hash: Mapped[str] = mapped_column(String(200)) # 200 characters for Argon2 hashes
    ...
    posts: Mapped[list[Post]] = relationship(
        back_populates="author", # Also we forward reference the Post model because it is defined after the User model, so we need to tell Python that it exists before we can use it in the relationship.
        cascade="all, delete-orphan") # The cascade all delete-orphan part makes sure if we delete an user, their posts will be deleted as well!
