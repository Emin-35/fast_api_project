...
from app.schemas import PostResponse, UserCreate, UserResponse, UserUpdate # Import the UserUpdate schema for updating users
...
@router.get("/{user_id}", response_model = UserResponse) # This route will be accessible at the URL "api/users/{user_id}" where {user_id} is a dynamic value that can be accessed in the route function.
async def get_user(
    user_id: int,
    db: Annotated[AsyncSession, Depends(get_db)],
    ):
