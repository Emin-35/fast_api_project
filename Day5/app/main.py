...
# .create_all is synchronous and we can't call synchronous methods with our async engine.
...
# Annotated metadata that tells FastAPI to inject a database session into the db parameter using the get_db dependency function. Does dependency injection.
# This allows us to interact with the database within our route function without having to manually create and manage the database session.
async def create_user(user: UserCreate, db: Annotated[AsyncSession, Depends(get_db)]): # For request body validation, we need UserCreate schema.
    result_username_search = await db.execute(
        select(models.User)
    ...
    result_email_search = await db.execute(
        select(models.User)
...
Result: Your code will crash, and you'll get a `MissingGreenlet` or `GreenletError`.
